...
def turn_off_touchscreen(device_name):
    call(['xinput', 'disable', device_name])
    
def turn_on_touchscreen(device_name):
    call(['xinput', 'enable', device_name])
    
def handle_touchscreen_events(device, device_name):
    for event in device.read_loop():
        if event.type == event.ecodes.EV_ABS:
            turn_on_touchscreen(device_name)

def print_input_device_name():
...
def main():
    ...
    print_input_device_name()
    
    touchscreen_device_name = 'event0'
    
    device = evdev.InputDevice(f'/dev/input/{touchscreen_device_name}')
    
    touchscreen_thread = threading.Thread(target=handle_touchscreen_events, args=(device, touchscreen_device_name))
    touchscreen_thread.start()
    
    try:
        time.sleep(60)
        turn_off_touchscreen(touchscreen_device_name)
        touchscreen_thread.join()
    except KeyboardInterrupt:
        pass
    finally:
        turn_on_touchscreen(touchscreen_device_name)
